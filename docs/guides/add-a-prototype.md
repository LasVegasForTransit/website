...
> **Before you start.** You need the website running locally (see [local development](../reference/local-dev.md)) and the Docket task the prototype belongs to.
...
```json
{
  "name": "Sign in",
  "status": "draft",
  "task": "01M35Q6QFMDSGCM0MBGXD1454E",
...
`task` is the Docket task's ID. `status` is one of `draft`, `testing`, `tested` or `built`. The prototype index at `/prototypes/` reads these files, so you never edit the index by hand.
...
- **tested** once the findings are posted on its Docket task and the blocking problems are fixed;
