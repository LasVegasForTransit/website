...
> **Before you start.** You need the prototype's preview link (see the prototype's Docket task), edit access to the tester roster in LVBT's shared drive (staff only), and 30 minutes per session.
...
Read this script aloud. Keep the prototype's tasks, which its Docket task lists, ready.
...
**Tasks.** Read each task from the prototype's Docket task, one at a time, for example "Join LVBT." Then watch. Don't help unless they have been stuck for two minutes; if you do help, note it.
...
Post the findings as a comment on the prototype's Docket task, in this format. Use session numbers, never names or emails.
