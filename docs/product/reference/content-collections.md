...
(see [glossary](../../development/reference/glossary.md#content-collection)). All site content lives
under `src/content/`. Schemas (the rules for what fields a file must have) are enforced by Zod (a
tool that checks data matches an expected shape — see
[glossary](../../development/reference/glossary.md#zod)) in `src/content.config.ts` — content that
doesn't match the shape will fail the build. That's deliberate: a typo in a content file stops the
...
see [glossary](../../development/reference/glossary.md#mdx)).
...
block at the top of an MDX file — see
[glossary](../../development/reference/glossary.md#frontmatter)) to author. The custom loader in
`src/lib/events-loader.ts` maps calendar fields to this validated shape:
...
Authoring lives in Google Calendar — see [`docs/guides/add-an-event.md`](../how-to/add-an-event.md)
...
[glossary](../../development/reference/glossary.md#rss)); there is no MDX to author. Set
`PUBLIC_LVBT_NEWSLETTER_FEED_URL` (and `PUBLIC_LVBT_NEWSLETTER_URL` for the "Read on Beehiiv" links)
— see [`.env.example`](../../../apps/site/.env.example). The loader in
`src/lib/newsletter-loader.ts` maps each feed `<item>` to this validated shape:
