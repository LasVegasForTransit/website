...
The Organizing Platform is the set of signed-in tools LVBT is adding to this website: joining and managing a membership, one record per supporter, event check-in, Discord roles, volunteer management and a staff console. The work is planned in the Organizing Platform Launch initiative in Docket, LVBT's planning tool. Each decision below has a short reason and a list of what it rules out.
...
- **ULID:** a unique identifier that sorts by the time it was created, for example `01M35Q2TBK5023XN8YW77PCMZS`.
...
- secrets in code, in the Wrangler configuration file or in Docket;
